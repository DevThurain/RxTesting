...
package com.thurainx.rxtesting.utils
...
import com.thurainx.rxtesting.connectivity.ConnectionState
...
fun ConnectivityManager.observeConnectivityAsObservable() : Observable<ConnectionState>{

    val basicApiCall = BehaviorSubject.create<ConnectionState>()
    basicApiCall.onNext(currentConnectivityState)

    basicApiCall
        .subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())

    basicApiCall.startWith(Observable.just(ConnectionState.Available))


    val callback = NetworkCallback { connectionState ->
        basicApiCall.onNext(connectionState)
    }
